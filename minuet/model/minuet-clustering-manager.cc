...
/*********************** ClusteringManager ******************************/
	...
	void ClusteringManager::StartClustering() {
		...
		switch (MinuetConfig::SELECTED_CLUSTERING_ALGORITHM) {
			case MinuetConfig::CLUSTERING_ALGORITHM::dca:
				...
				break;
            case MinuetConfig::CLUSTERING_ALGORITHM::rtt:
				if(m_rtt) {
					if(!m_rtt->IsStarted()) {
						m_rtt->StopClustering();
						PrintInLog("Starting Clustering Algorithm RTT Again");
					} else {
						NS_LOG_INFO("Impossible Start Clustering: Clustering already started!");
					}
				} else {
					m_rtt = CreateObjectWithAttributes<RTT>("Node", PointerValue(m_node), "CommunicationLayer", PointerValue(m_communicationLayer));
					m_rtt->StartClustering();
					PrintInLog("Starting Clustering Algorithm RTT");
				}
				break;
            /*
            ...
            case MinuetConfig::CLUSTERING_ALGORITHM::rtt:
				if(m_rtt) {
					if(m_rtt->IsStarted()) {
						...
						m_rtt->Dispose();
						m_rtt = nullptr;
						PrintInLog("Stopping Clustering Algorithm RTT");
					} else {
						NS_LOG_INFO("Impossible Stop Clustering: Clustering already stopped!");
					}
				} else {
	...
	bool ClusteringManager::IsRelay() {
		...
		switch (MinuetConfig::SELECTED_CLUSTERING_ALGORITHM) {
            ...
            case MinuetConfig::CLUSTERING_ALGORITHM::rtt:
				if(m_rtt) {
					return m_rtt->IsClusterHead();
				} else {
					cerr << "## ERROR: In IsClusterHead method RTT algorithm not started" << endl;
					exit(1);
				}
            /*
			case MinuetConfig::CLUSTERING_ALGORITHM::pctt:
	...
	bool ClusteringManager::IsClusterMember() {
		...
		switch (MinuetConfig::SELECTED_CLUSTERING_ALGORITHM) {
            ...
            case MinuetConfig::CLUSTERING_ALGORITHM::rtt:
				if(m_rtt) {
					return m_rtt->IsClusterMember();
				} else {
					cerr << "## ERROR: In IsClusterMember method RTT algorithm not started" << endl;
					exit(1);
				}
            /*
			case MinuetConfig::CLUSTERING_ALGORITHM::pctt:
	...
	bool ClusteringManager::IsIsolated() {
		...
		switch (MinuetConfig::SELECTED_CLUSTERING_ALGORITHM) {
            ...
            case MinuetConfig::CLUSTERING_ALGORITHM::rtt:
				if(m_rtt) {
					return m_rtt->IsIsolated();
				} else {
					cerr << "## ERROR: In IsIsolated method RTT algorithm not started" << endl;
					exit(1);
				}
            /*
			case MinuetConfig::CLUSTERING_ALGORITHM::pctt:
	...
	uint32_t ClusteringManager::GetRelayId(uint64_t op) {
		...
		switch (MinuetConfig::SELECTED_CLUSTERING_ALGORITHM) {
            ...
            case MinuetConfig::CLUSTERING_ALGORITHM::rtt:
				if(m_rtt) {
					//return m_rtt->GetRelayId();
                    return m_rtt->GetClusterId();
				} else {
					cerr << "## ERROR: In GetClusterId method RTT algorithm not started" << endl;
					exit(1);
				}
            /*
			case MinuetConfig::CLUSTERING_ALGORITHM::pctt:
