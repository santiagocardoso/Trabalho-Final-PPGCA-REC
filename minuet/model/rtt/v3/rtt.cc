...
void RTT::PromoteToClusterHead() {
    if (m_currentState == CANDIDATE) {
        m_currentState = CLUSTER_HEAD;
        m_clusterHeadId = m_myId;
        m_timeElectedAsCH = Simulator::Now();
...
void RTT::SendPeriodicMessage() {
    if (m_currentState == IDLE || m_currentState == CLUSTER_HEAD) {
        ...
        //PrintInLog("Sending UNICAST HEARTBEAT to CH " + std::to_string(m_clusterHeadId));
        RTTPacketHeader header;
...
void RTT::HandlePacket(Ptr<Packet> packet, Address from) {
    ...
    switch (header.GetMessageType()) {
        case DISCOVERY_MSG:
        {
            if (m_currentState == CANDIDATE && senderState == CLUSTER_HEAD) {
                Simulator::Cancel(m_promotionEvent);
                m_currentState = IDLE;
                PrintInLog("EVENT=CANCELLED_PROMOTION;REASON=CH_DISCOVERY_RECEIVED");
            }

            if (m_currentState == CLUSTER_MEMBER && senderId == m_clusterHeadId) {
                //PrintInLog("Received DISCOVERY from my CH. Updating contact time.");
                m_lastContactWithCH = Simulator::Now();
            }

            if (m_currentState == CLUSTER_HEAD && senderState == CLUSTER_HEAD && senderId < m_myId) {
                //PrintInLog("Conflict: Their CH ID (" + std::to_string(senderId) + ") is smaller. Renouncing.");
                Time gracePeriodEnd = m_timeElectedAsCH + Seconds(RTTUtils::GRACE_PERIOD);
                if (Simulator::Now() > gracePeriodEnd) {
                    m_currentState = IDLE;
                    m_clusterMembers.clear();
                    PrintInLog("EVENT=CH_RENOUNCED;CH_ID=" + std::to_string(m_myId) + ";REASON=CONFLICT");
                }
                else {
                    PrintInLog("EVENT=CONFLICT_IGNORED;REASON=GRACE_PERIOD;OPPONENT_ID=" + std::to_string(senderId));
                }
            }

            if (m_currentState == IDLE) {
                //PrintInLog("Received DISCOVERY from " + std::to_string(senderId) + ". Sending REPLY.");
        ...
        case REPLY_MSG:
        {
            ...
            //PrintInLog("Received REPLY from " + std::to_string(senderId) + ". RTT = " + std::to_string(rtt.GetSeconds()) + "s");
            ...
            if (rtt.GetSeconds() <= RTTUtils::RTT_THRESHOLD) {
                if (m_currentState == IDLE) {
                    //PrintInLog("RTT is GOOD. Auto-promoting to CLUSTER_HEAD.");
                    m_currentState = CANDIDATE;
                    PrintInLog("EVENT=BECAME_CANDIDATE;REASON=RTT_OK");

                    // Hesitação: agenda a promoção para um tempo aleatório entre 0 e 100ms
                    double hesitationTime = m_rand->GetValue() * 0.1; 
                    m_promotionEvent = Simulator::Schedule(Seconds(hesitationTime), &RTT::PromoteToClusterHead, this);
                }
                
                if (m_currentState == CLUSTER_HEAD && senderState == IDLE) {
                    //PrintInLog("I am CH and RTT from IDLE node " + std::to_string(senderId) + " is good. Sending INVITE.");
        ...
        case INVITE_MSG:
        {
            if ((m_currentState == IDLE || m_currentState == CANDIDATE) && senderState == CLUSTER_HEAD) { 
                //PrintInLog("Received INVITE from CH " + std::to_string(senderId) + ". Joining cluster.");
                if (m_currentState == CANDIDATE) {
                    Simulator::Cancel(m_promotionEvent);
                    PrintInLog("EVENT=CANCELLED_PROMOTION;REASON=INVITE_RECEIVED");
                }
        ...
        case HEARTBEAT_MSG:
        {
            if (m_currentState == CLUSTER_HEAD) {
                if (m_clusterMembers.find(senderId) == m_clusterMembers.end()) {
                    //PrintInLog("Received HEARTBEAT from a new member " + std::to_string(senderId) + ". Adding to list.");
                    PrintInLog("EVENT=MEMBER_JOIN;CH_ID=" + std::to_string(m_myId) + ";MEMBER_ID=" + std::to_string(senderId) + ";REASON=HEARTBEAT");
                }
                //else {
                //    PrintInLog("Received HEARTBEAT from member " + std::to_string(senderId) + ". Updating timestamp.");
                //}
    ...
    /*
    if (header.GetMessageType() == DISCOVERY_MSG)
        PrintInLog("Received a DISCOVERY message");
    else if (header.GetMessageType() == REPLY_MSG)
        PrintInLog("Received a REPLY message");
    */
}
...
void RTT::CleanUp() {
    if (m_currentState == CLUSTER_HEAD) {
        //PrintInLog("Running CleanUp. Current members: " + std::to_string(m_clusterMembers.size()));
        ...
        for (auto it = m_clusterMembers.begin(); it != m_clusterMembers.end(); ) {
            if (Simulator::Now() > (it->second + Seconds(RTTUtils::NEIGHBOR_EXPIRATION_TIME))) {
                //PrintInLog("Member " + std::to_string(it->first) + " expired. Removing.");
        ...
        if (Simulator::Now() > (m_lastContactWithCH + Seconds(RTTUtils::NEIGHBOR_EXPIRATION_TIME))) {
            //PrintInLog("Haven't heard from my CH " + std::to_string(m_clusterHeadId) + " for too long. Becoming IDLE again.");
