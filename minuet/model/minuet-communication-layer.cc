...
void CommunicationLayer::ReceiveControlMessage(Ptr<Socket> socket) {
	...
	while(packet = socket->RecvFrom(addr)) {
		...
		while (metadataIterator.HasNext()){
            ...
            } else if((item.tid.GetName() == "ns3::RTTPacketHeader") && m_rttInterface) {
				m_rttInterface->ReceiveControlMessage(packet, addr);
...
void CommunicationLayer::AttachRTTInterface(Ptr<RTTInterface> rttInterface) {
	m_rttInterface = rttInterface;
}
